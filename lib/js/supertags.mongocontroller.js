SuperTags.MongoController = new ClassX.extend(SuperTags.Class, function(base) {
  ...
  */
  ...
  this.addTagToDoc = function(docId, docTag) {
    ...
    if(this.docHasTags(itemDoc)) {
      ...
      if(itemDoc[this.parentObj] && itemDoc[this.parentObj][this.fieldName] && _.isArray(itemDoc[this.parentObj][this.fieldName])) {
        ...
        this.updateDocWithTags(docId, {$addToSet: itemSet});
        ...
        this.updateDocWithTags(docId, {$set: itemSet});
  ...
  this.removeTagFromDoc = function(docId, docTag) {
    ...
    itemSet[this.mongoField] = docTag
    if(_.isString(docTag)) {
      this.updateDocWithTags(docId, {$pull: itemSet});
    } else if(_.isArray(docTag)) {
      this.updateDocWithTags(docId, {$pullAll: itemSet});
    } else {
      return;
    }
  ...
  this.updateDocWithTags = function(docId, docModifier) {
    if(Meteor.isClient) {
      var ctx = this;
      var sFunc = {a: this }

      Meteor.call("SuperTags/updateDocWithTags-"+this.fieldName, docId, docModifier, function(error, callback) {
  ...
  */
  ...
  var dbSaveNewTag = function() {
    ...
    try{
      methods ={};
      methods['SuperTags/updateDocWithTags-' + options.tokenType] = dbTagUpdate.call(this);
      Meteor.methods(methods)
    } catch(ex) {};

    try {
      methods = {};
      methods["SuperTags/saveNewTag-"+options.tokenType] = dbSaveNewTag.call(this);
      Meteor.methods(methods);
    } catch(ex) {}
	}
});