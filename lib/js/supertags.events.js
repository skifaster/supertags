SuperTags.Events = ClassX.extend(SuperTags.Class, function(base){
  ...
  var textFilter = new ReactiveVar("");
...
console.log(ctx);
    ...
    console.log(containerElem.offsetTop)
    ...
    var itemTemplate = ctx.settings.autocomplete.itemTemplate || "supertagsAutocompleteItem";

    //data is a collection, need to filter with text provided in tag box
    if(ctx.settings.autocomplete.data && ctx.settings.autocomplete.data._collection) {
      Tracker.autorun(function() {
        var filter = {};
        filter[ctx.settings.autocomplete.lookupField] = {$regex: "^" + textFilter.get()};
        ctx._instCtx.autocompleteTextFilter.set(filter)
      })
    }
