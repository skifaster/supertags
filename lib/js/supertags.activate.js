SuperTags.Activate = ClassX.extend(SuperTags.Class, function(base) {
  ...
  this.parseAllTags = function(text) {
    ...
    this.tokenTypes.forEach(function (tokenType) {
      if(tokenType.parentObj) {
        console.log('have pt object')
        obj[tokenType.parentObj] = obj[tokenType.parentObj] || {};
        obj[tokenType.parentObj][tokenType.fieldName] = tokenType.parseTags(text)
      } else {
        obj[tokenType.fieldName] = tokenType.parseTags(text);
      }
    })
  ...
  this.parseAllTagsFromObject = function(obj) {
    this.tokenTypes.forEach(function (tokenType) {
      var text = obj[tokenType.parseField];
