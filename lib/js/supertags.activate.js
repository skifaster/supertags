  ...
  this.constructor = function Activate(options) {
    ...
    if(settings && settings.autocomplete && settings.autocomplete.rules) {
      settings.autocomplete.rules.forEach(function (rule) {
        ...
        ctx[rule.tokenType] = new SuperTags.Controller(subOptions);
        rule.callback = bindCallback.call(ctx[rule.tokenType], rule.callback);

        ctx[rule.tokenType]["mongoCRUD"] = new SuperTags.MongoController(subOptions);
